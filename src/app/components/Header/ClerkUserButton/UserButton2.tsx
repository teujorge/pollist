...
export function UserButton2() {
  ...
  return isSignedIn ? (
    <DropdownMenu>
      <DropdownMenuTrigger>
        <div className="flex h-9 w-9 select-none items-center justify-center rounded-full bg-transparent transition-colors hover:bg-accent focus:bg-accent active:bg-accent">
          <Image
          ...
          onClick={() =>
            openUserProfile({
              customPages: [
                {
                  label: "Settings",
                  mountIcon: (el: HTMLDivElement) => <>mountIcon</>,
                  unmountIcon: (el?: HTMLDivElement) => <>unMountIcon</>,
                  mount: (el: HTMLDivElement) => <>mount</>,
                  unmount: (el?: HTMLDivElement) => <>unmount</>,
                },
              ],
            })
          }
